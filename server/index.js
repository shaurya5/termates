...
httpServer.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`  ➜  Persist:  ${ptyManager.tmuxAvailable ? `${ptyManager.backend.name} (terminals survive restarts)` : 'none (install tmux for persistence)'}`);
  console.log('');
  console.log('  All data stays on this device. No telemetry.');
  console.log('');
});

// --- Shutdown: save state, detach PTYs, keep persistent sessions alive ---
function shutdown() {
  stopTuiMonitor();
  doPersist();
